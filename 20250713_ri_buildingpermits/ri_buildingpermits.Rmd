...
permit_rates <- bps %>%
  ...
  #pivot_wider(id_cols='geoidshort', values_from=`Total Units`, names_from=Year)
  #inner_join(ri_munis, by='geoidshort') %>% st_as_sf() %>%
  #inner_join(ri_munidata, by=join_by(geoidshort == GEOIDSHORT)) %>%
...
introduce why building more housing in higher density areas is an important policy goal (maybe cite Land Use 2025 for the density categories, plus ). Introduce the categories I chose and the population breakdown as a segue into the units permitted over time by town density category in RI.
...
p2 <- permit_rates %>%
  ...
  #geom_point(data=permit_rates %>% filter(str_starts(geoidshort, '25')), aes(color='black')) +
  geom_smooth(method='gam', se=FALSE, linetype='dashed', color='#bbb', linewidth=0.7) + 
  #geom_text(data=permit_rates %>% filter(name %in% towns_to_label), nudge_y=0.4) +
  #geom_vline(xintercept=1, linetype="dashed", color = "#bbbbbb") + 
  #geom_vline(xintercept=4, linetype="dashed", color = "#bbbbbb") +
  geom_text(x=6.5, y=6.5, label='Hover over each town for more detail') +
  lims(y=c(0,10)) +
  labs(color='Town Density Category', x='Housing Units per Acre', 
       ...
       title='RI towns with higher existing housing density tend to add new units at slower rates.') +
  scale_color_manual(values=town_density_colors) # TODO: use different colors than building type colors
...
while this is a remarkably tight trend for RI, it's not a general rule. Describe the high construction rates of high-density Boston-area munis. Also make the comparison to Worcester, maybe with a "competitiveness" framing because it's a similar city type & distance from Boston.
...
metro area as a whole in 2022's rate of 2.9 per existing 1000 units the lowest among metro areas of 1 million or more: https://rhodeislandcurrent.com/2023/08/18/providence-metro-sees-lowest-rate-of-home-construction-in-country-in-2022/
...
2024 the first year that more units were permitted in medium- and high-density towns than in low-density ones.

over 5000 permits a year in late 1980s to less than 1000 per year today: https://www.golocalprov.com/business/Despite-Hundreds-of-Millions-Providence-Metro-Ranks-Among-the-Worst-in-Dev
^is that statistic from a RI housing dept report on BPS data? maybe https://www.rihousing.com/wp-content/uploads/RIHousing-Presentation-final-04.2022.pdf 

but what kind of buildings?
...
p5 <- census_df %>%
  ...
  geom_errorbarh(aes(xmin = pct_hu_2plusf_lower, xmax = pct_hu_2plusf_upper,
                 ...
                 width=0.2) +
  geom_text(aes(x=0.5, y=1, label='Hover over each town for more detail')) +
...
Providence in 2024 permitted 269 units but only 60 in 5+ unit buildings: (238 - 60 = 178) in 2-4 unit bldgs.
Most of the multifamily permitted by PVD in 2019 and 2020 was also in 2-4 unit buildings, though not in 2022. 
...
2024 the first year since at least 2013 in which a majority of permitted units in RI were in multifamily buildings.

# Large Projects

introduce question of whether development is clustered in a few projects / areas or spread out, especially for 5+ unit buildings - the difference between dozens of six-plexes vs two or three very large apartment buildings. This is where I should acknowledge that a lot of the past two years' spike in medium density towns came from two large projects.

Doing some Googling based on these results, it seems that a lot of the credit for the spike in 5+ unit buildings and in medium-density towns is the 214-unit Alta Altitude project next to TF Green. Warwick housing reports indicate that there are two other similar-sized project also coming down the pipeline in that location near TF Green, being branded as "City Centre Warwick" (https://www.warwickri.gov/economic-development/pages/city-centre-warwick).

Adaptive reuse of Woonsocket Middle School into 150 units accounts for its 2023 spike (https://www.browndailyherald.com/article/2023/09/out-of-the-box-thinking-to-address-ris-housing-crisis-adaptive-reuse)

Without these individual projects, RI's housing pipeline would look pretty similar to previous years, mostly low-density towns and single-family units.

# Providence & 2-4 unit buildings

[why we should be interested in "missing middle"]
...
for (yyyy in first_year:2024) {
  ...
  tdfs[[as.character(yyyy)]] <- tdf %>%
    ...
    select(plat_lot_unit, class, class_desc) %>%
    mutate(year = yyyy)
}
...
pvd_res_housing_events <- pvd_parcels %>%
  ...
  inner_join(parcel_geoms, by='plat_lot_unit') %>% 
  st_as_sf() 
...
p8 <- pvd_hous_event_counts %>%
  ...
  # geom_bar(data=pvd_hous_event_counts %>% filter(event_type=='Constructed'), 
  #          aes(fill=building_type), stat='identity', position='stack') +
  geom_bar_pattern(#data=pvd_hous_event_counts %>% filter(event_type=='Demolished'), 
           aes(pattern=event_type, color=building_type), pattern_fill='black', 
           pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.01,
  ...
  lims(y=c(-50, 150)) + 
  labs(title='Residential construction in Providence has grown steadily from 2019 through 2024.',
...
https://pvdeye.org/getting-serious-about-providences-housing-crisis/ <- includes some stats about how much floor area would have to grow - maybe something to replicate in the context of city of PVD and think about how many vacant parcels are left
...
```{r}
shared_data <- SharedData$new(pvd_res_housing_events) # load data into crosstalk SharedData object
event_pal <- colorFactor(c('#4b4', '#f99'), c('Constructed', 'Demolished'))
